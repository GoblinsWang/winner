...
- `extern`可以用来声明一个变量或函数在外部定义的，让编译器**在链接时**（这样编译期就不会报“没有定义”的错误）寻找它的定义。
...
- 静态变量和全局变量都存放在全局数据区，但是**静态变量的作用域只限于定义它的文件或函数**，而全局变量的作用域是整个程序[1](https://blog.csdn.net/weixin_45525272/article/details/120402589)[2](https://www.runoob.com/note/36010)[3](https://zhuanlan.zhihu.com/p/64196844)。
- **静态变量只初始化一次，下一次使用时保持上一次的值**，而局部变量每次函数调用时都重新初始化[1](https://blog.csdn.net/weixin_45525272/article/details/120402589)[2](https://www.runoob.com/note/36010)[4](https://blog.csdn.net/zzhays/article/details/8011645)。
- 静态变量可以避免同名变量的冲突，提高模块间的耦合度[2](https://www.runoob.com/note/36010)[4](https://blog.csdn.net/zzhays/article/details/8011645)[5](https://www.cnblogs.com/2018shawn/p/9894920.html)。
- 静态函数只能在声明它的文件中使用，不能被其他文件调用，而普通函数没有这个限制[2](https://www.runoob.com/note/36010)[4](https://blog.csdn.net/zzhays/article/details/8011645)[5](https://www.cnblogs.com/2018shawn/p/9894920.html)。
...
C++中构造函数和析构函数都可以抛出异常，但是这样做很不安全，应该尽量避免，原因有以下几点：

- 如果构造函数抛出异常，那么对象就没有被成功构造，因此对象的析构函数就不会被调用，这可能导致**资源泄露或内存泄漏**。如果构造函数中分配了内存或其他资源，那么就应该在catch块中释放它们，或者使用智能指针等RAII技术来管理它们。
- 如果析构函数抛出异常，那么可能会导致程序过早结束或出现不明确的行为。如果析构函数中执行了某些必要的操作，比如释放某些资源，那么就应该在try-catch块中捕获并处理异常，或者直接忽略异常。
- 如果在栈展开的过程中析构函数抛出异常，那么可能会造成程序崩溃。栈展开是指当一个异常被抛出时，C++的异常处理机制会自动调用已经构造好的局部对象的析构函数来释放资源。如果析构函数本身也抛出异常，那么前一个异常尚未处理，又有新的异常，就会发生异常嵌套。如果没有合适的处理机制，就会导致terminate()函数被调用，程序终止。

所以，在C++中构造函数和析构函数中抛出异常是非常危险的做法，应该尽量避免。如果无法保证不发生异常，那么就应该在函数内部捕获并处理异常，或者使用RAII技术来管理资源。
...
- **内联函数**，因为它们是在编译时确定的，而虚函数是在运行时动态绑定的。内联函数语言多态的特性相违背。
...
- 调用operator new函数**为对象分配内存空间**，如果内存不足，会抛出`bad_alloc`异常或者返回0，除非使用`nothrow`或者`placement`版本的`operator new`。
- 在分配的内存空间上**调用类的构造函数**，初始化对象的成员变量和状态，如果构造函数抛出异常，会调用`operator delete`函数释放内存空间。
- 返回一个**指向新创建对象的指针**，可以通过该指针访问对象的成员和方法。
...
那么执行语句`A* a = new A(10);`的过程如下：

- 调用`operator new(size_t)`函数，传入参数`sizeof(A)`，申请一块大小为`sizeof(A)`的内存空间，假设返回地址为p。
- 调用`A::A(int)`构造函数，在地址p上创建一个A类型的对象，传入参数10，将成员变量x初始化为10。
- 返回地址p，赋值给指针a，可以通过`a->x`访问对象的成员变量x
...
- 类中有用户自定义的移动构造函数或移动赋值运算符重载。
- 类中有引用类型或const类型的成员变量，并且没有用户自定义的拷贝构造函数。
- 类中有不可复制或不可移动的成员变量（如`std::mutex,` `std::atomic`等）。

#### 9、拷贝构造函数与成员函数传入同类型的变量来进行拷贝的区别

拷贝构造函数和成员函数传入同类型的变量来进行拷贝的区别主要有以下几点：

- 拷贝构造函数是一种特殊的构造函数，它用于创建类对象的副本，而成员函数是一种普通的函数，它用于实现类的功能。
- 拷贝构造函数只有一个参数，即对同类型对象的引用，而成员函数可以有多个参数，也可以没有参数。
- 拷贝构造函数没有返回值，而成员函数可以有返回值，也可以没有返回值。
- 拷贝构造函数在以下情况下会被调用：通过使用另一个同类型的对象来初始化新创建的对象；复制对象把它作为参数传递给函数；复制对象，并从函数返回这个对象；类中有类类型的成员变量。而**成员函数在需要执行类的功能时会被调用（需要显式调用**）。
- 拷贝构造函数如果没有显示定义，编译器会提供一个默认的拷贝构造函数，它会按值拷贝类对象的所有成员变量。而成员函数如果没有显示定义，编译器不会提供任何默认的实现。
...
- vector支持连续的内存空间，因此可以实现更快的随机访问，而deque是分段连续的内存空间，随机访问的效率相比来说更低一些；

  > 连续分配内存，使得vector更符合空间局部性原理，方便内存读取。

- vector在尾部插入或删除元素时，只需要分配或释放内存，不需要移动元素，而deque在头部或尾部插入或删除元素时，可能需要**重新分配内存或调整指针**；

- vector在内存不足时，会一次性分配足够大的空间，并将原有元素复制过去，这样可以减少内存碎片和重新分配的次数，而deque在内存不足时，会分配一个新的段，并将其链接到原有段上，这样会增加内存碎片和重新分配的次数。
...
- 用户自定义字面量：允许用户为字面量定义自己的后缀运算符，以实现自定义的转换或计算。

- `noexcept`说明符和`noexcept`运算符：允许指定函数是否抛出异常，以及检查表达式是否抛出异常，用于提高异常安全性和性能。

- 多线程内存模型：定义了原子操作、顺序一致性、数据竞争等概念，用于支持并发编程。

- 并发支持库：提供了原子操作、互斥锁、条件变量、线程管理等功能，用于实现多线程编程。

- `std::unique_ptr`：提供了一种具有独占所有权语义的智能指针，用于替代`std::auto_ptr`。当然还有`shared_ptr`和`weak_ptr`智能指针。
